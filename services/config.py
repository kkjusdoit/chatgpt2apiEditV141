...
class ConfigStore:
    ...
    def image_poll_timeout_secs(self) -> int:
        try:
            return max(1, int(self.data.get("image_poll_timeout_secs", 100)))
        except (TypeError, ValueError):
            return 100
    ...
    @property
    def image_redundant_copies(self) -> int:
        try:
            return min(4, max(1, int(self.data.get("image_redundant_copies", 2))))
        except (TypeError, ValueError):
            return 2

    @property
    def image_redundant_max_attempts(self) -> int:
        try:
            return min(5, max(1, int(self.data.get("image_redundant_max_attempts", 3))))
        except (TypeError, ValueError):
            return 3
    ...
    @property
    def image_chat_panel_enabled(self) -> bool:
        """是否在生图页面显示文字聊天面板。"""
        value = self.data.get("image_chat_panel_enabled", True)
        if isinstance(value, str):
            return value.strip().lower() in {"1", "true", "yes", "on"}
        return bool(value)
    ...
    def get(self) -> dict[str, object]:
        ...
        data["image_redundant_generation_enabled"] = self.image_redundant_generation_enabled
        data["image_redundant_copies"] = self.image_redundant_copies
